...
// sharp uses `export = sharp` with a merged namespace. A default import binds
// the callable but not the namespace, so `sharp.Metadata` only resolved while
// a hoisted copy happened to be picked up differently. Import the types by
// name instead — stable regardless of where the package resolves from.

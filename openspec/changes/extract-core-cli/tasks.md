...
- [x] 17.1 Add parity fixture images to the repository:
  ...
  - **DEVIATION**: these are deterministic SYNTHETIC placeholders generated by `packages/nukebg-cli/scripts/generate-parity-fixtures.mjs` (pure math, no external assets/license), not real photographs — licensed portrait/product/logo photography was not sourceable in the implementing environment. See `packages/nukebg-core/tests/fixtures/parity/README.md` for full rationale and the Phase 18 follow-up needed (real fixtures + committed browser-baseline reference PNGs + cached models).

- [x] 17.2 Write parity test file:
  ...
  - Comparison math extracted to a pure, exported, independently-unit-tested helper: `packages/nukebg-core/src/parity/compare-alpha.ts` (`compareAlpha`), exported from `src/index.ts`, tested in `packages/nukebg-core/tests/parity/compare-alpha.test.ts` (12 hand-computed cases, green unconditionally — no model/fixture dependency).
  - **DEVIATION**: the model-gated body dynamically imports `NodePipelineRunner`/`OnnxNodeRmbgRunner`/`OnnxNodeLamaRunner` via a relative path into `nukebg-cli/src` (not the package barrel, which isn't built/exported yet) so the skip path never touches `nukebg-cli`'s build state. The forced (`NUKEBG_PARITY_REQUIRE=1`) path additionally requires a committed `<fixture>.reference-alpha.png` browser-baseline per fixture (REQ-PARITY-4); none exist yet (no drivable browser pipeline in this environment) so a forced run fails loudly with an actionable message rather than skipping silently or asserting against nothing. Tracked as Phase 18 follow-up.

- [x] 17.3 Verify parity test is skipped locally (no model cached) with the expected skip message. Verify it does NOT fail (just skip). Run `npm test`, expect green.
  - Confirmed: `Browser<->Node pixel parity (REQ-PARITY-1, REQ-PARITY-4) > Skipping parity test — RMBG model not cached (set NUKEBG_PARITY_REQUIRE=1 to force)` — 1 skipped, 0 failed.

- [x] 17.4 Verification: `npm test`, `npm run typecheck`. Milestone: "parity test framework in place; will enforce pixel equivalence in CI with cached models".
  - `npm test`: nukebg-app 737/737, nukebg-cli 76/76, nukebg-core 336/336 + 1 skipped (includes the new 12 `compare-alpha.test.ts` cases). `npm run typecheck`: all 3 workspaces green.
...
- [ ] **F.5 Cloudflare Pages build fails on the chain — MERGE BLOCKER, needs the build log**: the Pages check is `success` on both `dev` and `main` but `failure` on every chain PR from #291 onward, so the monorepo work broke the deploy. There is no `wrangler.toml` or any Pages config in the repo — the build command, root directory and output directory all live in the Cloudflare dashboard, and the check's log link is behind authentication this session cannot reach.
  - Why it matters: `CONTRIBUTING.md:239` requires the Pages preview to pass before merge, and once the tracker merges into `dev` production builds from the same config — so this breaks the live site, not just previews.
  - **First hypothesis, tested and REJECTED.** The obvious candidate was the output path: phase 2 moved the app under `packages/nukebg-app/`, and Vite resolves `outDir` against the project root, so the artifact moved from root `dist/` to `packages/nukebg-app/dist/` while the dashboard presumably still pointed at the old location. A workaround was pushed (`outDir: '../../dist'` + `emptyOutDir`, with matching `ci.yml` assertions) and **Pages still failed on the very commit that carried it** (`dc8ba00`). The output directory is therefore not the cause, or not the only one. The workaround was reverted — it changed the deploy contract on an unproven theory, and it actively conflicts with the preferred fix below (with the Pages root directory set to `packages/nukebg-app`, an `outDir` of `../../dist` writes outside that root).
  - **Current leading hypothesis, unverified:** the failure is in install/build, not artifact lookup. The check reports "Building — Build failed". Under npm workspaces a root install now also pulls `nukebg-cli`'s dependencies, which the browser app never needed: `onnxruntime-node` and `sharp`, both large native packages with install scripts. `dev` never installed those. Measured on a clean install: `onnxruntime-node` alone is **208 MB**, sharp's `@img/*` platform binaries another **20 MB**, against **829 MB** of total `node_modules` — so the chain adds roughly a quarter of a gigabyte of native payload to a build that previously only needed browser dependencies. That could exceed Pages' build time or disk limits, or fail on a native install step. Ruled out already: Node version pinning (neither `dev` nor the chain has `.nvmrc`/`.node-version`, so both get the same default) and the build command itself (`npm run build` produces a complete `dist/` — with `index.html`, JSON-LD and `assets/` — on GitHub Actions ubuntu, and `ci.yml`'s assertions pass on every chain PR).
  - **Next step is not a code change — read the log.** Open the Pages build log for a failing chain deployment and identify the failing step. If it is the install, the fix is to stop Pages installing the CLI's tree: set the Pages project's root directory to `packages/nukebg-app` (which also fixes the output path cleanly), or add an install command that scopes to the app workspace. If it is something else, the log will say.
  - Whatever the fix, update `README.md:97` ("Deploy `dist/`") in the same pass — it still documents the pre-monorepo layout.
...
**Merge blocker:** F.5 — the tracker must not merge to `dev` until the Cloudflare Pages build is green. The cause is still unidentified (the output-path theory was tested and rejected); diagnosing it needs the Pages build log, which requires dashboard access. Everything else in CI is green: all three OS legs, lint, typecheck, the full 1151-test suite, and the Playwright e2e projects.

...
**Still open (explicitly deferred, not blocking archive after this reconciliation):** 18.3 (CI 3-OS confirmation — needs a push), X.6 (see its note: coupled to the v1.1 parity baselines), F.1/F.2 above, and the v1.1 items (parity enforcement, stdin/stdout, `--json`, per-stage events). X.2–X.5 are now closed.

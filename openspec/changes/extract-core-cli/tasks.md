...
- [ ] **F.5 Cloudflare Pages build fails on the chain — MERGE BLOCKER, needs dashboard access**: the Pages check is `success` on both `dev` and `main` but `failure` on every chain PR, so the monorepo move broke the deploy. There is no `wrangler.toml` or any Pages config in the repo — the build command and output directory live in the Cloudflare dashboard, which this session cannot read (the check's log link is behind an authenticated dashboard). Best-supported diagnosis: Phase 2 moved the app under `packages/nukebg-app/`, and `vite.config.ts` sets `outDir: 'dist'` **relative to the Vite root**, so the artifact now lands at `packages/nukebg-app/dist/` while the dashboard almost certainly still points at root `dist/`. `README.md:97` still documents "Deploy `dist/`", which matches the stale config.
  - Why it matters now: `CONTRIBUTING.md:239` requires the Pages preview to pass before merge, and once the tracker merges into `dev` the production deploy builds from the same config — so this breaks the live site, not just previews.
  - **Workaround applied at the chain tip** (`cross-phase-hygiene`): `vite.config.ts` now sets `outDir: '../../dist'` with `emptyOutDir: true`, restoring the pre-monorepo contract with no dashboard change, and `ci.yml`'s build assertions follow. Verified: `dist/index.html` (JSON-LD present) and `dist/assets/` land at the repo root, nothing under `packages/nukebg-app/dist/`, and the full gate stays green (1151 tests, lint clean).
  - **Deliberately applied at the tip only, not at the chain root.** Propagating it to the root means rebasing all 16 descendant branches, and it is a workaround for a one-field dashboard setting — bad risk/benefit. Consequence: only the tracker's Pages check goes green; PRs #291 through #323 keep a red Pages preview. That is acceptable under feature-branch-chain, where only the tracker merges to `dev`, but reviewers should know the red is expected and already explained here.
  - **Preferred fix, still open:** set the Pages project's root directory to `packages/nukebg-app` (build command `npm run build`, output `dist`), then revert the `outDir` block to plain `'dist'` and restore `ci.yml`'s package-relative assertions. That keeps build output inside the package where it belongs. Update `README.md:97` in the same pass either way.

- [ ] **F.6 `CONTRIBUTING.md` is stale about the lint gate**: line 230 states the `Lint + format` job is "non-blocking today (`continue-on-error: true`)", but `.github/workflows/ci.yml` sets `continue-on-error: false` — it was flipped to blocking in #134 after the prettier sweep. One-line doc correction.

**Still open (explicitly deferred, not blocking archive after this reconciliation):** X.6 (see its note: coupled to the v1.1 parity baselines), F.1/F.2/F.4/F.6 above, and the v1.1 items (parity enforcement, stdin/stdout, `--json`, per-stage events). X.2–X.5, F.3 and 18.3 are now closed.

**Merge note:** F.5's workaround at the chain tip means the tracker's Pages build should pass, so the merge is no longer blocked. The earlier chain PRs keep a red Pages preview by design (see F.5) — confirm the tracker's Pages check is green before merging, and prefer the dashboard fix over keeping the workaround.

...
- [ ] **F.3 `onnxruntime-node` duplicated, breaks Linux CI (BLOCKER for 18.3)**: the first real push of this chain surfaced what local runs could not. `@huggingface/transformers@3.8.1` (nukebg-app) pins `onnxruntime-node` to **exactly `1.21.0`**, hoisted to the root `node_modules`; nukebg-cli asks for `^1.24.0` and gets `1.27.0` nested in `packages/nukebg-cli/node_modules`. On Linux the root napi-v3 `onnxruntime_binding.node` resolves the shared soname `libonnxruntime.so.1` to the nested 1.27 build, which does not export `VERS_1.21.0`, and `tests/cli.test.ts` + `tests/commands/process.test.ts` fail to load. macOS and Windows resolve differently and pass, which is why the matrix is green on two legs and red on one.
  - Attempted and rejected: a root `overrides` entry (npm refuses to apply an override to a package a workspace depends on directly unless the specs match exactly), aligning cli's spec to the override, and pinning cli to `1.21.0` — none deduped, npm kept the stale nested copy. The existing `protobufjs` override does work, so overrides are functional; this specific case is not yielding to them.
  - Needs a deliberate decision, and it is phase-19 scope (the `^1.24.0` bump), not part of the X.2–X.6 pass. Options: bump `@huggingface/transformers` to a release whose `onnxruntime-node` pin is compatible with the CLI's, drop the CLI to whatever transformers pins, or stop hoisting by isolating the CLI's dependency tree.

**Still open (explicitly deferred, not blocking archive after this reconciliation):** X.6 (see its note: coupled to the v1.1 parity baselines), F.1/F.2 above, and the v1.1 items (parity enforcement, stdin/stdout, `--json`, per-stage events). X.2–X.5 are now closed.

**Blocking:** 18.3 cannot be signed off until F.3 is resolved — the three-OS matrix now runs on every chain PR, and ubuntu is red.
